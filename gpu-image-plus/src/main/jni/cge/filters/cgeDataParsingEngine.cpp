...
do{\
	...
	CGEImageFilterInterface* CGEDataParsingEngine::adjustParser(const char* pstr, CGEMutipleEffectFilter* fatherFilter)
	{
		...
		*pBuffer = '\0';
		...
		else if(strcmp(buffer, "posterize") == 0)
        {
            ADJUSTHELP_COMMON_FUNC(pstr, CGEPosterizeFilter, setIntensity);
        }
        else if(strcmp(buffer, "vibrance") == 0)
        {
            ADJUSTHELP_COMMON_FUNC(pstr, CGEVibranceFilter, setIntensity);
        }
