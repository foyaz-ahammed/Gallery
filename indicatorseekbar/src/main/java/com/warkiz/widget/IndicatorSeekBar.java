...
public class IndicatorSeekBar extends View {
    ...
    private String[] mTickTextsCustomTextArray;
    //indicator
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mShowTrack = ta.getBoolean(R.styleable.IndicatorSeekBar_isb_show_track, builder.showTrack);
        //thumb
        mShowThumb = ta.getBoolean(R.styleable.IndicatorSeekBar_isb_show_thumb, builder.showThumb);
        ...
        mKeyTicksCount = ta.getInt(R.styleable.IndicatorSeekBar_isb_key_ticks_count, builder.keyTickCount);
...
//        mTickTextsCustomTextArray = ta.getTextArray(R.styleable.IndicatorSeekBar_isb_tick_texts_array);
...
//        if (mTicksCount < mKeyTicksCount) {
//            throw new IllegalArgumentException("the Argument: KEY TICK COUNT must be smaller than TICK COUNT");
//        }
    ...
    private void initStrokePaint() {
        if (mStockPaint == null) {
            ...
            mStockPaint.setStyle(Paint.Style.FILL);
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSize(SizeUtils.dp2px(mContext, 170), widthMeasureSpec), height + 10);
        initSeekBarInfo();
...
//        }
    ...
    private void initTextsArray() {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            if (mShowTickText) {
                ...
                if (mDegree == 0 || mDegree == 180) if (i == 0) mTextCenterX[i] += mRect.width() / 2.0f - Math.min(20, mRect.width() / 2.0f);
            }
    ...
    private String getTickTextByPosition(int index) {
        if (mTickTextsCustomTextArray == null) {
            return getProgressString(mProgressArr[index]);
        }
        if (index < mTickTextsCustomTextArray.length) {
            return String.valueOf(mTickTextsCustomTextArray[index]);
        }
        return "";
    }
...
//            canvas.drawLine(mProgressTrack.left, mProgressTrack.top, mProgressTrack.right, mProgressTrack.bottom, mStockPaint);
            ...
            mStockPaint.setStrokeWidth(1);
            mStockPaint.setPathEffect(new DashPathEffect(new float[]{5, 10}, 0));
            canvas.drawLine(mProgressTrack.left, mBackgroundTrack.top, mBackgroundTrack.right, mBackgroundTrack.bottom, mStockPaint);

            mStockPaint.setPathEffect(null);
        }
    }
    ...
    private void drawTickMarks(Canvas canvas) {
        ...
        for (int i = 0; i < mTickMarksX.length; i++) {
            ...
            if (mShowTickMarksType == TickMarkType.OVAL) {
                float thumbLeft = thumbCenterX - mThumbTouchRadius;
                float thumbRight = thumbCenterX + mThumbTouchRadius;
//                float tickTextRight = mTextCenterX[i] + mTickTextsWidth[i] / 2.0f;
//                float lastTickTextLeft = mTextCenterX[mTickMarksX.length-1] - mTickTextsWidth[mTickMarksX.length-1] / 2.0f;
                if (!mShowThumb) {
                    if (i % mKeyTicksCount == 0 || i == mTickMarksX.length - 1) {
//                        if (tickTextRight < lastTickTextLeft - 20 || i == mTickMarksX.length - 1) {
                        if (i == mTickMarksX.length - 1) {
                            canvas.drawCircle(mTickMarksX[i], mProgressTrack.top, mTickRadius, mStockPaint);
                        } else {
                            canvas.drawCircle(mTickMarksX[i], mProgressTrack.top, mTickRadius - 2, mStockPaint);
                        }
                    } else {
                        canvas.drawCircle(mTickMarksX[i], mProgressTrack.top, mTickRadius - 2, mStockPaint);
                    }
                } else {
                    if (mTickMarksX[i] + mTickRadius < thumbLeft || mTickMarksX[i] - mTickRadius > thumbRight) {
                        if (i % mKeyTicksCount == 0 || i == mTickMarksX.length - 1) {
//                            if (tickTextRight < lastTickTextLeft - 20 || i == mTickMarksX.length - 1) {
                            if (i == mTickMarksX.length - 1) {
                                canvas.drawCircle(mTickMarksX[i], mProgressTrack.top, mTickRadius, mStockPaint);
    ...
    public void showTickText(boolean showTickText) {
        mShowTickText = showTickText;
        invalidate();
    }

    private void drawTickTexts(Canvas canvas) {
        if (mTickTextsArr == null || !mShowTickText) {
            return;
        }
        ...
        for (int i = 0; i < mTickTextsArr.length; i++) {
            ...
            float tickTextLeft = mTextCenterX[index] - mTickTextsWidth[index] / 2.0f;
            float tickTextRight = mTextCenterX[index] + mTickTextsWidth[index] / 2.0f;
            float thumbTextLeft =  thumbCenterX - mTickTextsWidth[getClosestIndex()] / 2.0f - 10;
            float thumbTextRight = thumbCenterX + mTickTextsWidth[getClosestIndex()] / 2.0f + 10;
            float lastTickTextLeft = mTextCenterX[mTickMarksX.length-1] - mTickTextsWidth[mTickMarksX.length-1] / 2.0f;

            if (mDegree == 90) {
                tickTextLeft = mTextCenterX[index] - mTickTextsHeight / 2.0f;
                tickTextRight = mTextCenterX[index] + mTickTextsHeight / 2.0f;
                thumbTextLeft =  thumbCenterX - mTickTextsHeight / 2.0f - 10;
                thumbTextRight = thumbCenterX + mTickTextsHeight / 2.0f + 10;
            }

            canvas.rotate(mDegree, mTextCenterX[i], mTickTextY);

            if (mIsTouching) {
                if (i % mKeyTicksCount == 0 || i == 0 || i == mTickTextsArr.length - 1) {
                    if (tickTextRight < lastTickTextLeft - 20 || i == mTickTextsArr.length - 1) {
                        if (mDegree == 90) canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2, mTickTextY, mTextPaint);
                        else if (mDegree == 270) canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2, mTickTextY, mTextPaint);
                        else canvas.drawText(mTickTextsArr[index], mTextCenterX[i], mTickTextY, mTextPaint);
                    }
                }
            } else {
                if (!mShowThumb){
                    ...
                    if (tickTextLeft > thumbTextRight || tickTextRight < thumbTextLeft) {
                        if (i % mKeyTicksCount == 0 || i == 0 || i == mTickTextsArr.length - 1) {
                            if (tickTextRight < lastTickTextLeft - 20 || i == mTickTextsArr.length - 1) {
                                if (mDegree == 90) canvas.drawText(mTickTextsArr[index], mTextCenterX[i] - mTickTextsWidth[index] / 2, mTickTextY, mTextPaint);
                                else if (mDegree == 270) canvas.drawText(mTickTextsArr[index], mTextCenterX[i] + mTickTextsWidth[index] / 2, mTickTextY, mTextPaint);
    ...
    public void setTickTextRotation(int degree) {
        mDegree = degree;
        initTextsArray();
        invalidate();
    }
    private void drawThumb(Canvas canvas) {
        ...
        if (mThumbDrawable != null) {//check user has set thumb drawable or not.ThumbDrawable first, thumb color for later.
            ...
            mStockPaint.setStyle(Paint.Style.FILL);
            canvas.drawCircle(thumbCenterX, mProgressTrack.top, 3, mStockPaint);

            mStockPaint.setStyle(Paint.Style.STROKE);
            mStockPaint.setStrokeWidth(3.0f);
    ...
    private void drawThumbText(Canvas canvas) {
        ...
        canvas.rotate(mDegree, getThumbCenterX(), mThumbTextY);

        String thumbText = getIndicatorTextString();
        mTextPaint.getTextBounds(thumbText, 0, thumbText.length(), mRect);

        if (mDegree == 90) canvas.drawText(thumbText, getThumbCenterX() - mRect.width() / 2.0f, mThumbTextY, mTextPaint);
        else if (mDegree == 270) canvas.drawText(thumbText, getThumbCenterX() + mRect.width() / 2.0f, mThumbTextY, mTextPaint);
        else canvas.drawText(thumbText, getThumbCenterX(), mThumbTextY, mTextPaint);
...
//                }
    ...
    private float getClosestValue() {
        ...
        return mProgressArr[closestIndex];
    }
    ...
    private void apply(Builder builder) {
        ...
        this.mTickTextsCustomTextArray = builder.tickTextsCustomArray;
    ...
    String getIndicatorTextString() {
        if (mTickTextsCustomTextArray != null) {
            return mTickTextsCustomTextArray[getClosestIndex()];
        } else {
...
//        return getProgressString(mProgress);
    ...
    public int getProgressIdx() {
        return getClosestIndex();
    }

    public float getClosestProgress() {
...
//        }
    ...
    public synchronized void setProgressIdx(int idx) {
        lastProgress = mProgress;
        mProgress = mProgressArr[idx];
     ...
     */
    public void customTickTexts(@NonNull List<String> tickTextsArr) {
        if (tickTextsArr == null) {
            mTickTextsCustomTextArray = null;
            return;
        }
        mTickTextsCustomTextArray = new String[tickTextsArr.size()];
        if (mTickTextsArr != null) {
            for (int i = 0; i < mTickTextsArr.length; i++) {
                mTickTextsCustomTextArray[i] = tickTextsArr.get(i);
            }
            initTextsArray();
            invalidate();
        }
    ...
    public void setKeyTickCount(int keyTickCount) {
        mKeyTicksCount = keyTickCount;
    }
    /**
