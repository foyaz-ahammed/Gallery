package com.kr.commons.dialogs
...
import com.kr.commons.adapters.FilepickerItemsAdapter
import com.kr.commons.extensions.*
import com.kr.commons.helpers.DATE_FORMAT_ONE
import com.kr.commons.helpers.Helper
import com.kr.commons.helpers.SORT_BY_SIZE
import com.kr.commons.helpers.ensureBackgroundThread
import com.kr.commons.models.FileDirItem
import com.kr.commons.views.Breadcrumbs
...
class FilePickerDialog(val activity: BaseSimpleActivity,
    ...
    private val mDateFormat = DATE_FORMAT_ONE
    ...
    init {
        ...
        mDialogView.filepicker_fab_show_hidden.apply {
            beVisibleIf(canAddShowHiddenButton)
        ...
        Helper.adjustAlertDialogWidth(mDialog, activity)
        if (!pickFile) {
    ...
    private fun getItems(path: String, getProperFileSize: Boolean, callback: (List<FileDirItem>) -> Unit) {
        if (activity.isPathOnOTG(path)) {
            activity.getOTGItems(path, getProperFileSize, callback)
        } else {
    ...
    private fun getRegularItems(path: String, getProperFileSize: Boolean, callback: (List<FileDirItem>) -> Unit) {
        ...
        for (file in files) {
            ...
            val size = if (getProperFileSize) file.getProperSize() else file.length()
            items.add(FileDirItem(curPath, curName, file.isDirectory, file.getDirectChildrenCount(), size, file.lastModified()))
        }
        callback(items)
    }
