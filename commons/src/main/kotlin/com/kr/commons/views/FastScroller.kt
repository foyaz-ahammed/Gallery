...
import com.kr.commons.extensions.getAdjustedPrimaryColor
import com.kr.commons.extensions.onGlobalLayout

// based on https://blog.stylingandroid.com/recyclerview-fastscroll-part-1

/**
 * 등록부보기화면에서 scroll할때 오른쪽에 보여지는 scroller이다.
 * scroller를 끌기하면 그옆에 현재 위치의 media의 촬영 날자,시간이 현시된다.
 */
class FastScroller : FrameLayout {
    ...
    private fun setPosition(pos: Float) {
        if (isHorizontal) {
            ...
            if (bubble != null && handle!!.isSelected) {
                bubble!!.y = getValueInRange(0, recyclerViewHeight - bubbleHeight, handle!!.y)
                bubbleHideHandler.removeCallbacksAndMessages(null)
                bubble?.alpha = 1f
            }
        }
    ...
    private fun getValueInRange(min: Int, max: Int, value: Float) = min.toFloat().coerceAtLeast(value).coerceAtMost(max.toFloat())
}
