package com.kr.commons.asynctasks
...
class CopyMoveTask(val activity: BaseSimpleActivity, val copyOnly: Boolean, val copyMediaOnly: Boolean, val conflictResolutions: LinkedHashMap<String, Int>,
                   listener: CopyMoveListener) : AsyncTask<Pair<ArrayList<FileDirItem>, String>, Void, Boolean>() {
    private val INITIAL_PROGRESS_DELAY = 3000L
    private val PROGRESS_RECHECK_INTERVAL = 500L

    private var mListener: CopyMoveListener
    ...
    override fun doInBackground(vararg params: Pair<ArrayList<FileDirItem>, String>): Boolean? {
        ...
        for (file in mFiles) {
            if (file.size == 0L) {
                file.size = file.getProperSize(activity)
            }
        ...
        for (file in mFiles) {
            try {
                ...
                if (activity.getDoesFilePathExist(newPath) && !activity.needsSdcardWritePermission(file.path)) {
    ...
    override fun onPostExecute(success: Boolean) {
        ...
        if (success) {
            mListener.onSucceeded(copyOnly, (mFileCountToCopy - mTransferredFiles.size).coerceAtLeast(0))
        } else {
            mListener.onFailed()
        }
    }
    ...
    private fun copyFile(source: FileDirItem, destination: FileDirItem) {
        if(activity.needsSdcardWritePermission(source.path)) {
            mCurrentProgress += source.size
            return
        }
        if(activity.needsSdcardWritePermission(destination.path.getParentPath())) {
            mCurrentProgress += source.size
            return
        }
        ...
        try {
            ...
            if (source.size == copiedSize && activity.getDoesFilePathExist(destination.path)) {
                ...
                if (!copyOnly) {
                    inputStream.close()
                    out?.close()
                    activity.deleteFileWithPath(source.path, true)
                }
