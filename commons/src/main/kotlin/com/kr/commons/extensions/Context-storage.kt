...
fun Context.isPathOnSD(paths: List<String>) = paths.any { isPathOnSD(it) }

fun Context.isPathOnOTG(paths: List<String>) = paths.any { isPathOnOTG(it) }
...
/**
 * @param pathList 화일경로들
 * @return SD Card에 대한 쓰기권한이 필요되는가를 돌려준다
 */
fun Context.needsSdcardWritePermission(pathList: List<String>): Boolean {
    for (path in pathList) {
        if(isPathOnSD(path)) {
            //사용자가 권한을 접수하면 treeUri에 SDCard경로가 들어간다.
            if(baseConfig.treeUri == "")
                return true

            //App정보에서 사용자가 권한을 다시 없앨수 있으므로 실지 DocumentFile을 얻을수 있는가 따져본다.
            return getDocumentFile(path) == null
        }
    }

    return false
}
fun Context.needsSdcardWritePermission(path: String) = needsSdcardWritePermission(arrayListOf(path))
 ...
 * 화일지우기
 * @param filePath 화일경로
 * @param updateMediaStore MediaStore에서 삭제를 진행하겠는가? 숨김화일지우기할때는 본래 MediaStore에 없었으므로 삭제할 필요가 없다.
 */
fun Context.deleteFileWithPath(filePath: String, updateMediaStore: Boolean):Boolean {
    var fileDeleted = true

    //SD card의 화일지우기
    if(isPathOnSD(filePath)) {
        fileDeleted = tryFastDocumentDelete(filePath, false)
        if (!fileDeleted) {
            val document = getDocumentFile(filePath)
            if (document != null) {
                try {
                    fileDeleted = DocumentsContract.deleteDocument(applicationContext.contentResolver, document.uri)
    ...
    if(fileDeleted && updateMediaStore) {
        //Media store에서 화일 삭제
        val where = "${MediaStore.MediaColumns.DATA} = ?"
        val args = arrayOf(filePath)
        contentResolver.delete(getFileUri(filePath), where, args)
    }
    return fileDeleted
}
...
fun Context.rescanPaths(paths: List<String>, callback: (() -> Unit)? = null) {
    ...
    for (i in paths.indices)
        mimeTypes[i] = paths[i].getFilenameFromPath()

    MediaScannerConnection.scanFile(applicationContext, paths.toTypedArray(), mimeTypes) { s, uri ->
        callback?.invoke()
    }
}
...
fun Context.getOTGItems(path: String, getProperFileSize: Boolean, callback: (ArrayList<FileDirItem>) -> Unit) {
    ...
    for (file in files) {
        ...
        val fileSize = when {
            getProperFileSize -> file.getItemSize()
            isDirectory -> 0L
            else -> file.length()
        }
